...
then
  ...
  lanip=$(ndmc -c 'show interface Bridge0' | grep address | awk '{print $2}')
	...
	wget --no-check-certificate -O /opt/etc/ndm/fs.d/100-ipset.sh https://raw.githubusercontent.com/corex-cpu/bypass/master/100-ipset.sh
	...
	mkdir /opt/tmp/tor
	wget --no-check-certificate -O /opt/etc/tor/torrc https://raw.githubusercontent.com/corex-cpu/bypass/master/torrc
	#sed -i "s/hash:net/${set_type}/g" /opt/etc/tor/torrc
	echo "Установлены настройки Tor"
	
	wget --no-check-certificate -O /opt/etc/shadowsocks.json https://raw.githubusercontent.com/corex-cpu/bypassc/master/shadowsocks.json
	...
	wget --no-check-certificate -O /opt/etc/unblocksh.txt https://raw.githubusercontent.com/corex-cpu/bypass/master/unblocksh.txt
	wget --no-check-certificate -O /opt/etc/unblocktor.txt https://raw.githubusercontent.com/corex-cpu/bypass/master/unblocktor.txt
	echo "Установлены сайты и ip адреса для обхода блокировок для обоих обходов"

	wget --no-check-certificate -O /opt/bin/unblock_ipset.sh https://raw.githubusercontent.com/corex-cpu/bypass/master/unblock_ipset.sh
	chmod +x /opt/bin/unblock_ipset.sh
	echo "Установлен скрипт для заполнения множеств unblock IP-адресами заданного списка доменов"
	
	
	wget --no-check-certificate -O /opt/bin/unblock_dnsmasq.sh https://raw.githubusercontent.com/corex-cpu/bypass/master/unblock_dnsmasq.sh
	...
	wget --no-check-certificate -O /opt/bin/unblock_update.sh https://raw.githubusercontent.com/corex-cpu/bypass/master/unblock_update.sh
	...
	wget --no-check-certificate -O /opt/etc/init.d/S99unblock https://raw.githubusercontent.com/corex-cpu/bypass/master/S99unblock
	chmod +x /opt/etc/init.d/S99unblock
	#sed -i "s/hash:net/${set_type}/g" /opt/etc/init.d/S99unblock
	#sed -i "s/10.15.5.1/${lanip}/g" /opt/etc/init.d/S99unblock
	echo "Установлен cкрипт автоматического заполнения множества unblock при загрузке маршрутизатора"
	
	wget --no-check-certificate -O /opt/etc/ndm/netfilter.d/100-redirect.sh https://raw.githubusercontent.com/corex-cpu/bypass/master/100-redirect.sh
	chmod +x /opt/etc/ndm/netfilter.d/100-redirect.sh
	sed -i "s/hash:net/${set_type}/g" /opt/etc/ndm/netfilter.d/100-redirect.sh
	sed -i "s/10.15.5.1/${lanip}/g" /opt/etc/ndm/netfilter.d/100-redirect.sh
	echo "Установлено перенаправление пакетов с адресатами из unblock в Tor и Shadowsocks"
	
	rm -rf /opt/etc/dnsmasq.conf
	wget --no-check-certificate -O /opt/etc/dnsmasq.conf https://raw.githubusercontent.com/corex-cpu/bypass/master/dnsmasq.conf
	sed -i "s/10.15.5.1/${lanip}/g" /opt/etc/dnsmasq.conf
	echo "Установлена настройка dnsmasq и подключение дополнительного конфигурационного файла к dnsmasq" 
	
	rm -rf /opt/etc/crontab
	wget --no-check-certificate -O /opt/etc/crontab https://raw.githubusercontent.com/corex-cpu/bypass/master/crontab
